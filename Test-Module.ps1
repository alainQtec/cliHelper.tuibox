...
#!/usr/bin/env pwsh
# .SYNOPSIS
#   cliHelper.tuibox testScript v0.1.0
# .EXAMPLE
#   ./Test-Module.ps1 -version 0.1.0
#   Will test the module in ./BuildOutput/cliHelper.tuibox/0.1.0/
# .EXAMPLE
#   ./Test-Module.ps1
#   Will test the latest  module version in ./BuildOutput/cliHelper.tuibox/
param (
  ...
  [ValidateScript({
      ...
      $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.tuibox')
...
begin {
  $TestResults = $null
  $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.tuibox'))
  ...
  $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.tuibox', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
  if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "cliHelper.tuibox.psd1"))
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "cliHelper.tuibox.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
  $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.tuibox.Integration.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.tuibox.Features.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.tuibox.Module.Tests.ps1')))
}
